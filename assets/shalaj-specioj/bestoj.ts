...
export interface Besto {
  ...
  nivelo?: number;     // akvosurfaco por METITAJ bestoj ( la objekta ilo ) —
                       // la enLago-vojo uzas gxin anstataŭ la komuna lago
...
export function konstruiBestojn(sceno: THREE.Scene,
  ...
  for ( let i = 0; i < kvanto && pentritaj; i++ ) {
    ...
    const y = enLago && lago ? lago.nivelo + bazaY : akvoYFn(x) + bazaY;
...
export function konstruiMetitanBeston(sceno: THREE.Scene,
  ...
  sceno.add(grupo);
  return {
    grupo, korpo, vosto, animajxoj, bazajKruroj, x, zOfseto: 0, cz: z, enLago: true,
    nivelo: akvoY, bazaY,
    direkto: Math.random() * Math.PI * 0o2,
    phase: Math.random() * Math.PI * 0o2,
    amplitudo: 0o3, rapido: 0o1/0o4 + Math.random() * 0o3/0o10,
    speco: speco.speco,
  };
}
...
export function gxisdatigiBestojn(s: BestoSistemo, t: number): void {
  for ( const b of s.bestoj ) {
    ...
    if ( b.speco === "beroe" ) {
      ...
      if ( b.vosto ) b.vosto.rotation.y = Math.sin(t * 0o4 + b.phase) * 0o3/0o10;
